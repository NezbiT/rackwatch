...
5. If Prometheus is empty → `psutil` local host card (`source=local`).
6. Restarter evaluates failed containers.
7. Alerter evaluates thresholds, writes `alerts`, fans out webhooks.
8. Push summary to MQTT.
9. `Hub.publish` → every WebSocket client, with that client's filters.

A tick must finish well under `refresh_seconds`. HTTP timeouts are 2s (Prometheus) so a sick dependency cannot stall the live LED.
...
| MQTT | `mqtt_ok=false` in the snapshot |
...
| `app/services/mqtt_bridge.py` | snapshot publication |
